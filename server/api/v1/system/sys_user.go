...
func (b *BaseApi) Login(c *gin.Context) {
	...
	if !oc || (l.CaptchaId != "" && l.Captcha != "" && store.Verify(l.CaptchaId, l.Captcha, true)) {
		...
		b.TokenNext(c, *user)
		userService.AddLoginHistory(&system.LoginHistory{
			UserID:    int(user.ID),
			LoginTime: time.Now(),
		})
		return
...
// @Tags      SysUser
// @Summary   获取用户登录信息
// @Param     data body request.LoginHistoryRequest true "用户ID和时间范围"
// @Security  ApiKeyAuth
// @accept    application/json
// @Produce   application/json
// @Success   200   {object}  response.Response{data=map[string]interface{},msg=string}  "获取用户登录信息"
...
func (b *BaseApi) GetLoginHistoryByIdAndTimeRange(c *gin.Context) {
	...
	ReqLog, err := userService.GetLoginHistory(req.UserID, startTime, endTime)
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
		return
	...
	for _, log := range ReqLog {
		formattedLogs = append(formattedLogs, gin.H{
			"userId":    log.UserID,
			"loginTime": log.LoginTime.Format("2006年01月02日 15时04分05秒"),
		})
	}

	response.OkWithDetailed(gin.H{"loginHistory": formattedLogs}, "获取成功", c)
}

// DeleteLoginHistoryByIdAndTimeRange
// @Tags      SysUser
// @Summary   删除用户登录信息
// @Param     data body request.LoginHistoryRequest true "用户ID和时间范围"
// @Security  ApiKeyAuth
// @accept    application/json
// @Produce   application/json
// @Success   200   {object}  response.Response{data=map[string]interface{},msg=string}  "删除用户登录信息"
// @Router    /user/deleteLoginHistoryByIdAndTimeRange [post]
func (b *BaseApi) DeleteLoginHistoryByIdAndTimeRange(c *gin.Context) {
	var req request.LoginHistoryRequest
	...
	err := userService.DeleteLoginHistory(req.UserID, startTime, endTime)
	if err != nil {
		global.GVA_LOG.Error("删除失败!", zap.Error(err))
		response.FailWithMessage("删除失败", c)
		return
