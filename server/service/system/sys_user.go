...
func (userService *UserService) AddLoginHistory(log *system.LoginHistory) (err error) {
	return global.GVA_DB.Create(&log).Error
}

func (userService *UserService) GetLoginHistory(id int, startTime, endTime time.Time) (log []system.LoginHistory, err error) {
	if id != 0 {
		if !startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ? AND login_time BETWEEN ? AND ?", id, startTime, endTime).Find(&log).Error
		}
		if !startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ? AND login_time >= ?", id, startTime).Find(&log).Error
		}
		if startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ? AND login_time <= ?", id, endTime).Find(&log).Error
		}
		if startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ?", id).Find(&log).Error
		}
	} else {
		if !startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("login_time BETWEEN ? AND ?", startTime, endTime).Find(&log).Error
		}
		if !startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Where("login_time >= ?", startTime).Find(&log).Error
		}
		if startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("login_time <= ?", endTime).Find(&log).Error
		}
		if startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Find(&log).Error
		}
	}
	return
}

func (userService *UserService) DeleteLoginHistory(id int, startTime, endTime time.Time) (err error) {
	if id != 0 {
		if !startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ? AND login_time BETWEEN ? AND ?", id, startTime, endTime).Delete(&system.LoginHistory{}).Error
		}
		if !startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ? AND login_time >= ?", id, startTime).Delete(&system.LoginHistory{}).Error
		}
		if startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ? AND login_time <= ?", id, endTime).Delete(&system.LoginHistory{}).Error
		}
		if startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Where("user_id = ?", id).Delete(&system.LoginHistory{}).Error
		}
	} else {
		if !startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("login_time BETWEEN ? AND ?", startTime, endTime).Delete(&system.LoginHistory{}).Error
		}
		if !startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Where("login_time >= ?", startTime).Delete(&system.LoginHistory{}).Error
		}
		if startTime.IsZero() && !endTime.IsZero() {
			err = global.GVA_DB.Where("login_time <= ?", endTime).Delete(&system.LoginHistory{}).Error
		}
		if startTime.IsZero() && endTime.IsZero() {
			err = global.GVA_DB.Delete(&system.LoginHistory{}).Error
		}
	}
	return
}
